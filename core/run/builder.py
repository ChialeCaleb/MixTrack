...
from .running_loop import RunnerDriver, get_model
import pdb
...
def _build_model(config, runtime_vars, building_context: BuildingContext):
    ...
    if weight_path is not None:
        try:
            ...
            model_state_dict = torch.load(weight_path, map_location='cpu')['model']
            for key in list(model_state_dict.keys()):
                model_state_dict.update({key[7:]: model_state_dict.pop(key)})
...
def build_running_tasks(runtime_vars, config: dict, global_rng, local_rng, wandb_instance):
    ...
    if runtime_vars.mixin_config:
        model = get_model(building_context.model)
    else:
        device_ids = config['gpus']
        model = torch.nn.DataParallel(building_context.model, device_ids=device_ids)

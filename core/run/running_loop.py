...
class RunnerDriver:
    def __init__(self, name, n_epochs, model, runs, event_dispatcher, runtime_vars, efficiency_assessor, wandb_instance,
        ...
        self.istrained = runtime_vars.istrained
        self.device = torch.device(runtime_vars.device)

    def run(self):
        if self.resume_path is not None and self.istrained:
            model_state_dict = torch.load(self.resume_path)
            for key in list(model_state_dict['model'].keys()):
                model_state_dict['model'].update({'module.' + key: model_state_dict['model'].pop(key)})
            assert model_state_dict['version'] == 2
            self.model.load_state_dict(model_state_dict['model'])
            self.model.module._freeze()

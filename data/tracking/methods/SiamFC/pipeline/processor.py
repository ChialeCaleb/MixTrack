...
class SiamTrackerProcessor:
    ...
    def __call__(self, z_image, z_event, z_bbox, x_image, x_event, x_bbox, is_positive):
        data = {}
        data['is_positive'] = is_positive
        if z_image is x_image:
            z_image = x_image = z_image.to(torch.float)
            z_event = x_event = z_event.to(torch.float)
        else:
            z_image = z_image.to(torch.float)
            x_image = x_image.to(torch.float)
            z_event = z_event.to(torch.float)
            x_event = x_event.to(torch.float)
        ...
        z_curated_event, _ = do_SiamFC_curation(z_event, self.template_size, z_curation_parameter, self.interpolation_mode)
        x_curated_event, _ = do_SiamFC_curation(x_event, self.search_size, x_curation_parameter, self.interpolation_mode)
        ...
        if np.random.random() < self.gray_scale_probability:
            ...
            z_curated_event = self.gray_scale_transformer(z_curated_event)
            x_curated_event = self.gray_scale_transformer(x_curated_event)            

        z_curated_image = self.transform(z_curated_image)
        x_curated_image = self.transform(x_curated_image)
        z_curated_event = self.transform(z_curated_event)
        x_curated_event = self.transform(x_curated_event)        

        data.update({
            'z_curated_image': z_curated_image,
            'x_curated_image': x_curated_image,
            'z_curated_event': z_curated_event,
            'x_curated_event': x_curated_event,
        })
...
class SiamFCBatchDataCollator:
    ...
    def __call__(self, data_list):
        ...
        if 'z_curated_image' in first_element:
            ...
            z_curated_event = _collate(data_list, 'z_curated_event', True)
            x_curated_event = _collate(data_list, 'x_curated_event', True)
            samples_batched_on_device = z_curated_image, x_curated_image, z_curated_event, x_curated_event

...
from .network import MixTrack
from data.tracking.methods.SiamFC.pseudo_data import build_siamfc_pseudo_data_generator


def build_mix_track_main_components(config, num_epochs, iterations_per_epoch, event_register: EventRegister, has_training_run):
...
def build_mix_track(config, load_pretrained, num_epochs, iterations_per_epoch, event_register: EventRegister, has_training_run):
    ...
    encoder, decoder, out_norm, z_backbone_out_stage, x_backbone_out_stage, z_input_projection, x_input_projection, z_pos_enc, x_pos_enc = \
        build_mix_track_main_components(config, num_epochs, iterations_per_epoch, event_register, has_training_run)
    head = build_head(config)

    return MixTrack(backbone, encoder, decoder, out_norm, head, z_backbone_out_stage, x_backbone_out_stage, z_input_projection, x_input_projection, z_pos_enc, x_pos_enc), \

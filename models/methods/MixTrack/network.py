...
class MixTrack(nn.Module):
    ...
    def forward(self, z_image, x_image, z_event, x_event, z_feat=None):
        """
        Combined entry point for training and inference (include initialization and tracking).
            ...
            """
        if z_feat is None:
            z_feat = self.initialize(z_image)
        if x_image is not None:
            return self.track(z_feat, x_image)
        else:
            return z_feat
    ...
    def _freeze(self, backbone=True, encoder=True, decoder=True, out_norm=True, head=True, 
                x_input_projection=True, z_input_projection=True, 
                x_pos_enc=True, z_pos_enc=True):
        if backbone:
        ...
        if out_norm:
            for param in self.out_norm.parameters():
                param.requires_grad = False
        try:
            if x_input_projection:
                for param in self.x_input_projection.parameters():
                    param.requires_grad = False
        except:
            print('x_input_projection frozen failed')
        try:
            if x_pos_enc:
                for param in self.x_pos_enc.parameters():
                    param.requires_grad = False
        except:
            print('x_pos_enc frozen failed')
        try:
            if z_pos_enc:
                for param in self.z_pos_enc.parameters():
                    param.requires_grad = False
        except:
            print('z_pos_encn frozen failed')
        try:            
            if z_input_projection:
                for param in self.z_input_projection.parameters():
                    param.requires_grad = False
        except:
            print('z_input_projection frozen failed')
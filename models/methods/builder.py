...
def build_model(config: dict, runtime_vars, max_batch_size, num_epochs: int, iterations_per_epoch: int, event_register: EventRegister, has_training_run: bool):
    ...
    if config['version'] == 1 and config['type'] == 'MixTrack':
        from .MixTrack.builder import build_mix_track
        return build_mix_track(config, load_pretrain, num_epochs, iterations_per_epoch, event_register, has_training_run)
    else:
